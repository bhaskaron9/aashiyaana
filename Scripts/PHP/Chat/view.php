...
foreach ($fetch as $row):
	...
	if (($row['username'] == $username) && ($time == $now)) {
		$user = '<strong style="color:green;">'.$row['username'].'</strong>'; 
	}else{
	...
	echo '<p>'.$user.':<em>('.$hourAndMinutes.')</em>'.'<br/>'.' '.'<img src="spechbubble_sq_line.png" width="10" height="10">'.' '. $row['message_content']. '</p>';
